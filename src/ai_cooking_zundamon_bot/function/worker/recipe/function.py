...
from ai_cooking_zundamon_bot.common.ai.zudamon import ask_zunda
from ai_cooking_zundamon_bot.common.aws.bedrock import converse, embed
from ai_cooking_zundamon_bot.common.aws.s3 import download, upload
from ai_cooking_zundamon_bot.common.aws.ssm import get_parameter
from ai_cooking_zundamon_bot.common.discord import get_option_dict, respond_interaction
from ai_cooking_zundamon_bot.common.event.aczb import ACZBCommand
from ai_cooking_zundamon_bot.common.faiss import delete
from ai_cooking_zundamon_bot.common.util import get_zunda_error_comment
...
def handler(event: dict, context: dict) -> None:
    try:
        for record in event["Records"]:
            ...
            if command != ACZBCommand.RECIPE.value:
                raise ValueError(
                    f"This Lambda only supports '/{ACZBCommand.RECIPE.value}' [command: /{command}]"
                )
        ...
        respond_interaction(
            get_zunda_error_comment(exception),
...
def _handle_request(request: dict) -> str:
    ...
    bucket_name = get_parameter(key="/ACZB/VECTOR_STORE_BUCKET_NAME")
    ...
    match action:
        ...
        case "none":
            return "レシピ関連の操作や質問以外の内容は受け付けられないのだ!!別の聞き方をしてほしいのだ!!\n"
        case _:
            return "そんなコマンド知らないのだ!!よく分からないからボクの開発者に聞いてほしいのだ!!\n"
...
def _handle_regist_action(
    ...
    if response["stopReason"] == "tool_use":
        ...
        return (
            f"新しく{recipe_name}のレシピを覚えたのだ!!\n"
            f"レシピについて聞きたくなったら、いつでもボクに聞いてほしいのだ!!"
        )
    else:
        return (
            f"レシピに関する情報がうまく取れなかったのだ...\n"
            f"ボクへの命令文に以下の内容が含まれているかチェックしてほしいのだ!!\n"
            f"```\n"
            f"- レシピの名前\n"
            f"- レシピの材料(数量なども含むのだ!!)\n"
            f"- レシピの手順\n"
...
def _handle_ask_action(
    ...
    if recipes:
        ...
        return (
            f"まだボクは何もレシピを覚えていないのだ!!\n"
            f"ボクに新しくレシピを覚えるよう、先に命令してほしいのだ!!"
        )
...
def _handle_list_action(recipes: List[Dict]) -> str:
    if recipes:
        ...
        return (
            f"ボクが覚えているレシピの一覧なのだ!!\n"
            f"ここに載っていないレシピがあれば、ボクに命令してレシピを教えてほしいのだ!!\n"
            f"```\n"
            f"{formatted_recipes}\n"
            f"```"
        )
...
def _handle_delete_action(
    ...
    if recipes:
        ...
        if delete_id is None:
            return (
                f"ボクの記憶に「{recipe_name}」という名前のレシピが見当たらないのだ...\n"
                f"レシピの名前を確認して、もう一度削除を命令してほしいのだ!!"
            )
        ...
        return (
            f"ボクの記憶から{recipe_name}のレシピを削除したのだ!!\n"
            f"レシピを覚えるのは大変なんだから、ボクにあまり忘れさせないでほしいのだ!!\n"
        )
    else:
        return (
            f"まだボクは何もレシピを覚えていないのだ!!\n"
